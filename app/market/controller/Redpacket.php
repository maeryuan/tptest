...
use app\market\model\RedpacketModel;
...
//        return $this->redirect('Redpacket/listRedpacketModel');
    ...
    public function issueRedpacket() {
        $redpacketModelId = input('param.redpacket_model_id');
        $issueNums = input('param.issue_nums');
        #issue_nums如果不是整数或者是负数，则返回输入的值错误
        if (!is_numeric($issueNums) || $issueNums < 0) {
            return json(msg(-100, '', '发放红包的个数必须是正整数'));
        }
        #更新redpacket_record中的issue_nums字段的值
        $res = RedpacketModel::where(['redpacket_model_id' => $redpacketModelId])->update(['issue_nums' => $issueNums,'update_time'=>time()]);
       
        if (!$res) {
            return json(msg(-1, '', '操作失败'));
        }
        return json(msg(200, '发送红包的个数为：' . $issueNums, '发送红包成功'));
    }
